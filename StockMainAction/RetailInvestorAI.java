...
public class RetailInvestorAI {
    ...
    public void makeDecision(Stock stock, OrderBook orderBook, StockMarketSimulation simulation) {
        ...
        if (!Double.isNaN(sma)) {
            ...
            if (this.shouldIgnoreThreshold()) {
                // 當忽略門檻時，有一定機率進行買賣操作
                if (cash >= currentPrice && priceDifferenceRatio < 0 && actionProbability > 0.3) {
                    int maxAffordableAmount = (int) (cash / currentPrice);
                ...
                } else if (stocksOwned > 0 && priceDifferenceRatio > 0 && actionProbability > 0.3) {
                    int sellAmount = (int) (stocksOwned * priceDifferenceRatio * (0.5 + 0.5 * random.nextDouble()));
                    sellAmount = Math.max(1, Math.min(sellAmount, stocksOwned)); // 限制賣出量範圍
                ...
                if (priceDifferenceRatio < -dynamicBuyThreshold && cash >= currentPrice && actionProbability > 0.2) {
                    int maxAffordableAmount = (int) (cash / currentPrice);
                    int buyAmount = (int) (maxAffordableAmount * Math.abs(priceDifferenceRatio) * (0.5 + 0.5 * random.nextDouble()));
                    buyAmount = Math.max(1, Math.min(buyAmount, 500)); // 限制買入量範圍
                ...
                } else if (priceDifferenceRatio > dynamicSellThreshold && stocksOwned > 0 && actionProbability > 0.2) {
                    int sellAmount = (int) (stocksOwned * priceDifferenceRatio * (0.5 + 0.5 * random.nextDouble()));
                    sellAmount = Math.max(1, Math.min(sellAmount, stocksOwned)); // 限制賣出量範圍
                    ...
                    if (random.nextBoolean() && cash >= currentPrice) {
                        int buyAmount = random.nextInt(50) + 1;
                        buyStock(stock, buyAmount, orderBook, "隨機買入操作。\n");
                    } else if (stocksOwned > 0) {
                        int sellAmount = random.nextInt(stocksOwned) + 1;
    ...
    private void sellStock(Stock stock, int amount, OrderBook orderBook, String decisionReason) {
        if (stocksOwned >= amount) {
            ...
            // 可以選擇輸出賣出操作資訊
            // System.out.println(decisionReason);
            // System.out.println(String.format("散戶下賣單 %d 股，價格 %.2f，剩餘持股 %d 股", amount, price, stocksOwned));
        } else {
