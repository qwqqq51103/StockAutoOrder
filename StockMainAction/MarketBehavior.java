...
public class MarketBehavior {
    ...
    public void marketFluctuation(Stock stock, OrderBook orderBook, double volatility, int recentVolume) {
        ...
        // 2. 考慮均值回歸，調整幅度更小
        double meanReversionSpeed = 0.005; // 減小均值回歸速度
        ...
        if (random.nextDouble() < 0.01) { // 1% 機率發生突發事件
            eventImpact = currentPrice * (0.02 * (random.nextDouble() - 0.5)); // 價格變動 ±1%
        }
        ...
        if (priceChangeRatio > 0) {
            ...
            // 檢查是否有足夠的庫存來執行賣單
            if (sellVolume > 0 && account.freezeStocks(sellVolume)) {
                // 創建並提交賣單
