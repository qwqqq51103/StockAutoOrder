...
import StockMainAction.view.order.OrderBookSnapshot;
import StockMainAction.view.order.OrderTableModel;
import StockMainAction.view.order.OrderViewerCoordinator;
import javax.swing.*;
...
public class OrderViewer extends JFrame {
    ...
    private final OrderViewerCoordinator coordinator;
    private ComponentAdapter lifecycleComponentListener;
    private WindowStateListener lifecycleWindowStateListener;
    ...
    // 各分頁的表格模型和表格
    private OrderTableModel allOrdersModel;
    private OrderTableModel buyOrdersModel;
    private OrderTableModel sellOrdersModel;
    private OrderTableModel myOrdersModel;
    ...
    public OrderViewer(OrderBook orderBook) {
        ...
        initializeUI();
        this.coordinator = new OrderViewerCoordinator(orderBook, this::applySnapshot);
        installRefreshLifecycle();
        this.coordinator.refreshNow();
    }
    ...
    private void initializeUI() {
        setTitle("訂單管理中心");
        WindowSizing.apply(this, new Dimension(1200, 800),
                new Dimension(800, 550), null);
    ...
    private void installRefreshLifecycle() {
        lifecycleComponentListener = new ComponentAdapter() {
            @Override
        ...
        lifecycleWindowStateListener = event -> updateWindowActiveState();
        addComponentListener(lifecycleComponentListener);
        addWindowStateListener(lifecycleWindowStateListener);
    }

    private void updateWindowActiveState() {
        boolean minimized = (getExtendedState() & Frame.ICONIFIED) != 0;
        coordinator.setWindowActive(isShowing() && !minimized);
    }
    ...
    private void createTabbedPane() {
        ...
        // 創建全部訂單分頁
        allOrdersModel = createTableModel();
        ...
        // 創建買入訂單分頁
        buyOrdersModel = createTableModel();
        ...
        // 創建賣出訂單分頁
        sellOrdersModel = createTableModel();
        ...
        // 創建我的訂單分頁
        myOrdersModel = createTableModel();
     ...
     * 創建表格模型
     */
    private OrderTableModel createTableModel() {
     ...
     * 創建美化的表格
     */
    private JTable createStyledTable(OrderTableModel model) {
    ...
    private JPanel createBottomPanel() {
        ...
        autoRefreshCheckBox.addActionListener(e -> {
            if (autoRefreshCheckBox.isSelected()) {
                coordinator.setAutoRefreshEnabled(true);
            } else {
                coordinator.setAutoRefreshEnabled(false);
            }
        });
        ...
        refreshButton.addActionListener(e -> coordinator.refreshNow());
    ...
    private class ButtonRenderer extends JButton implements TableCellRenderer {
        ...
        public Component getTableCellRendererComponent(JTable table, Object value,
            ...
            setEnabled("待成交".equals(status) || "部分成交".equals(status));
...
// 新增按鈕編輯器類（在 OrderViewer 類別內部）
     ...
     * 加載所有訂單
     */
    private void applySnapshot(OrderBookSnapshot snapshot) {
        allOrdersModel.applyRows(allOrdersTable, snapshot.allOrders());
        buyOrdersModel.applyRows(buyOrdersTable, snapshot.buys());
        sellOrdersModel.applyRows(sellOrdersTable, snapshot.sells());
        myOrdersModel.applyRows(myOrdersTable, snapshot.personalOrders());

        // 更新統計信息
        updateStatistics(snapshot.buys().size(), snapshot.sells().size());
     ...
     * 更新統計信息
     */
    private void updateStatistics(int buyOrderCount, int sellOrderCount) {
        totalOrdersLabel.setText("總訂單數: " + (buyOrderCount + sellOrderCount));
        buyOrdersLabel.setText("買單: " + buyOrderCount);
        sellOrdersLabel.setText("賣單: " + sellOrderCount);
    }
    ...
    public void dispose() {
        if (disposed) return;
        disposed = true;
        if (lifecycleComponentListener != null) {
            removeComponentListener(lifecycleComponentListener);
            lifecycleComponentListener = null;
        }
        if (lifecycleWindowStateListener != null) {
            removeWindowStateListener(lifecycleWindowStateListener);
            lifecycleWindowStateListener = null;
        }
        coordinator.close();
        super.dispose();
    }
    ...
    private void searchTable(JTable table, String searchText) {
        if (searchText == null || searchText.trim().isEmpty()) {
            currentSorter(table).setRowFilter(null);
            return;
        }

        TableRowSorter<OrderTableModel> sorter = currentSorter(table);
    ...
    private void filterTable(JTable table, String filterType) {
        ...
        if ("所有交易者".equals(filterType)) {
            ...
            sorter.setRowFilter(new RowFilter<OrderTableModel, Integer>() {
                @Override
                public boolean include(Entry<? extends OrderTableModel, ? extends Integer> entry) {
    ...
    private TableRowSorter<OrderTableModel> currentSorter(JTable table) {
        return (TableRowSorter<OrderTableModel>) table.getRowSorter();
    }
}
