...
public class MainView extends JFrame {
    ...
    // [PERF] 圖表合併重繪排程參數
    private final ChartUpdateCoordinator chartUpdates = new ChartUpdateCoordinator(120);
    private final UiUpdateCoordinator uiUpdates = new UiUpdateCoordinator(25);
    private final UiThemeManager themeManager = new UiThemeManager();
    ...
    private final java.util.ArrayDeque<InfoEvent> pendingInfoEvents = new java.util.ArrayDeque<>();
    private static final int MAX_PENDING_INFO_EVENTS = 1_000;
    private static final int MAX_INFO_CHARACTERS = 200_000;
    ...
    private final java.util.ArrayDeque<TapeEvent> pendingTapeEvents = new java.util.ArrayDeque<>();
    private static final int MAX_PENDING_TAPE_EVENTS = 500;
    private record TapeEvent(boolean buyerInitiated, double price, int volume,
    ...
    public void dispose() {
        ...
        if (marketStatsTimer != null) {
            marketStatsTimer.stop();
            marketStatsTimer = null;
        }
    ...
    private void initializeUI() {
        ...
        WindowSizing.apply(this, new Dimension(1600, 900),
                new Dimension(1100, 700), null);
    ...
    private void openSettingsDialog() {
        if (!SwingUtilities.isEventDispatchThread()) {
            uiUpdates.submit("settings-dialog", this::openSettingsDialog);
            return;
        }
        if (settingsDialog == null || !settingsDialog.isDisplayable()) {
            ...
            tabs.addTab("主題", createSettingsTabTheme());
            tabs.addTab("日誌", createSettingsTabLog());
            tabs.addTab("效能", createSettingsTabPerf());
            tabs.addTab("命令", createSettingsTabCommand());
            tabs.addTab("字級", createSettingsTabFont());
            tabs.addTab("K線跟隨", createSettingsTabKlineFollow());
            tabs.addTab("指標設定", createSettingsTabIndicators());
            tabs.addTab("事件模式", createSettingsTabEventMode());
            tabs.addTab("滑價", createSettingsTabSlippage());
            tabs.addTab("撤換間隔", createSettingsTabReplaceInterval());
            tabs.addTab("主力限制", createSettingsTabMainForceLimits());
            tabs.addTab("重置", createSettingsTabReset());
            root.add(tabs, BorderLayout.CENTER);
            settingsDialog = new SettingsDialog(this, root);
        }
        settingsDialog.showAndFocus();
    }
    ...
    public void scheduleChartFlush() {
        chartUpdates.requestFlush();
    }

    // [PERF] 提供外部（ControlView）切換效能模式介面
    public void applyPerfMode(String mode) {
        int ms;
        if ("節能".equals(mode)) ms = 200; else if ("效能".equals(mode)) ms = 60; else ms = 120;
        chartUpdates.setDelay(ms);
    }
...
//                    syncChartsInVisiblePanel(cardPanel);
    ...
    public void updateRetailInfoTable(java.util.List<StockMainAction.model.RetailInvestorAI> investors, double stockPrice) {
        uiUpdates.submit("retail-info", () -> {
            retailInfoTableModel.setRowCount(0);
    ...
    public void updateTraderInfoTable(java.util.List<StockMainAction.model.StockMarketModel.TraderSnapshot> snapshots) {
        uiUpdates.submit("trader-info", () -> {
            if (traderInfoTableModel == null) return;
            traderInfoTableModel.setRowCount(0);
    ...
    public void updateRetailProfitChart(java.util.List<StockMainAction.model.RetailInvestorAI> investors, double stockPrice, double defaultInitial) {
        uiUpdates.submit("retail-profit", () -> {
    ...
    public void updatePriceChart(int timeStep, double price, double sma) {
        latestViewState.updateAndGet(state -> state.withPrice(timeStep, price, sma));
        enqueuePriceUpdate(new PriceUpdate(timeStep, price, sma, System.currentTimeMillis()));
        uiUpdates.submit("price-chart", this::flushPriceChartUpdates);
    }
    ...
    private void applyPriceChartUpdate(int timeStep, double price, double sma, long timestampMillis) {
    ...
    public void updateVolumeChart(int timeStep, int volume) {
        latestViewState.updateAndGet(state -> state.withVolume(timeStep, volume));
        enqueueVolumeUpdate(new VolumeUpdate(timeStep, volume, System.currentTimeMillis()));
        uiUpdates.submit("volume-chart", this::flushVolumeChartUpdates);
    }
    ...
    private static void flushOnEdtAndWait(Runnable flush) {
        ...
        try {
            ...
            throw new IllegalStateException("Interrupted while applying UI backpressure", exception);
        } catch (java.lang.reflect.InvocationTargetException exception) {
            throw new IllegalStateException("Failed to flush chart updates", exception.getCause());
        }
    }
    ...
    private void applyVolumeChartUpdate(int timeStep, int volume, long timestampMillis) {
    ...
    private static class TapePanel extends JPanel {
        ...
        private final TradeTapeMetrics metrics = new TradeTapeMetrics(
                10_000L, java.time.Clock.systemDefaultZone());
        public TapePanel(){
        ...
        public void pushTrade(boolean buyerInitiated, double price, int volume, double bestBid, double bestAsk){
            ...
            long timestamp = metrics.record(buyerInitiated, volume, slipAbs);
            String t = new java.text.SimpleDateFormat("HH:mm:ss").format(new java.util.Date(timestamp));
        ...
        private void recompute(){
            TradeTapeMetrics.Snapshot stats = metrics.snapshot();
            if (stats.tradeCount() == 0){ ratioLabel.setText("買/賣比: -- / --"); rateLabel.setText("近10s: 0.0 筆/秒  0 量/秒"); slipLabel.setText("均滑價: --"); avgVolLabel.setText("均量: --"); streakLabel.setText("最大連續: --"); return; }
            ratioLabel.setText(String.format("買/賣比: %.1f%% / %.1f%%", stats.buyPercentage(), stats.sellPercentage()));
            rateLabel.setText(String.format("近10s: %.2f 筆/秒  %,d 量/秒", stats.tradesPerSecond(), Math.round(stats.volumePerSecond())));
            slipLabel.setText(String.format("均滑價: %.2f", stats.averageSlippage()));
            avgVolLabel.setText(String.format("均量: %.1f", stats.averageVolume()));
            streakLabel.setText(String.format("最大連續: 買%d / 賣%d", stats.maximumBuyStreak(), stats.maximumSellStreak()));
        }
    }
    ...
    public void pushTapeTrade(boolean buyerInitiated, double price, int volume, double bestBid, double bestAsk){
        synchronized (pendingTapeEvents) {
            if (pendingTapeEvents.size() >= MAX_PENDING_TAPE_EVENTS) {
                pendingTapeEvents.removeFirst();
            }
            pendingTapeEvents.addLast(new TapeEvent(buyerInitiated, price, volume, bestBid, bestAsk));
        }
        uiUpdates.submit("trade-tape", this::flushTapeTrades);
    }

    private void flushTapeTrades() {
        if (tapePanel == null) return;
        java.util.List<TapeEvent> batch;
        synchronized (pendingTapeEvents) {
            batch = new java.util.ArrayList<>(pendingTapeEvents);
            pendingTapeEvents.clear();
        }
        for (TapeEvent event : batch) {
            tapePanel.pushTrade(event.buyerInitiated(), event.price(), event.volume(),
                    event.bestBid(), event.bestAsk());
        }
    }
    
    // 對外公開：手動更新信號指示器面板
    public void updateSignalIndicators() {
        uiUpdates.submit("signal-indicators", () -> {
            try {
                if (signalPanel != null) {
                    int bullCount = bullSignals != null ? bullSignals.getItemCount() : 0;
                    int bearCount = bearSignals != null ? bearSignals.getItemCount() : 0;
    ...
    public void updateMainForceStatus(String phase, double recentTrend) {
        uiUpdates.submit("main-force-status", () -> {
    ...
    public void showErrorMessage(String message, String title) {
        Runnable show = () -> JOptionPane.showMessageDialog(this, message, title, JOptionPane.ERROR_MESSAGE);
        if (SwingUtilities.isEventDispatchThread()) show.run(); else SwingUtilities.invokeLater(show);
    }
    ...
    public void showInfoMessage(String message, String title) {
        Runnable show = () -> JOptionPane.showMessageDialog(this, message, title, JOptionPane.INFORMATION_MESSAGE);
        if (SwingUtilities.isEventDispatchThread()) show.run(); else SwingUtilities.invokeLater(show);
    }
    ...
    public void appendToInfoArea(String message, InfoType type) {
        synchronized (pendingInfoEvents) {
            if (pendingInfoEvents.size() >= MAX_PENDING_INFO_EVENTS) {
                pendingInfoEvents.removeFirst();
            }
            pendingInfoEvents.addLast(new InfoEvent(message, type));
        }
        uiUpdates.submit("info-log", this::flushInfoEvents);
    }

    private void flushInfoEvents() {
        if (infoTextArea == null) return;
        java.util.List<InfoEvent> batch;
        synchronized (pendingInfoEvents) {
            batch = new java.util.ArrayList<>(pendingInfoEvents);
            pendingInfoEvents.clear();
        }
        StringBuilder formatted = new StringBuilder();
        for (InfoEvent event : batch) {
            ...
            formatted.append(timestamp).append(' ').append(prefix).append(event.message()).append('\n');
        }
        infoTextArea.append(formatted.toString());
        int overflow = infoTextArea.getDocument().getLength() - MAX_INFO_CHARACTERS;
        if (overflow > 0) {
            try {
                infoTextArea.getDocument().remove(0, overflow);
